...
class DWA():
    def __init__(self,
                 ...
                 sim_time = 2.0, #simulation time for each trajectory
                 time_granularity = 0.1, #time granularity for simulation
                 v_samples = 10, #how many values of linear velocity to sample
                 w_samples = 20, #how many values of angular velocity to sample
                 #so you will simulate v_samples * w_samples trajectories
                 #it's better to have an higher number of angular samples than linear samples 
                 goal_dist_tol = 0.3, # distance tolerance to the goal to consider it reached (30 cm)
                 collision_tol = 0.3, # distance tolerance to obstacles to consider a collision (30 cm)
                 weight_angle = 0.04,
                 weight_vel = 0.2,
                 weight_obs = 0.1, #these are the weights for the objective function
                 obstacles_map = None, # A list of obstacles poses in the map # TO DO: accept a gridmap and use local map
                 # you should consider lidar as your sensor to detect obstacles and get the data in an obstacle list
                 **kwargs
                 ):

        self.dt = dt
        self.sim_step = round(sim_time / time_granularity)
        self.robot = Differential_drive_robot(**kwargs) #just to have a function that update the state of the robot, but in the lab you don't need this because you get the data from the odometry topic
    ...
    def go_to_pose(self, goal_pose): #core of the program, this generates command to reach a goal
        """
        ...
        if goal_pose is list:
            goal_pose = np.array(goal_pose) #assign a goal to the robot
        ...
        while steps <= self.max_num_steps:
            ...
            if dist_to_goal < self.goal_dist_tol:
                ...
                print("Goal reached!")
                break
            # we are verifyng if we are close or in to the goal
            # 2. Get new observations for obstacles is needed
            # no lidar used here, be carefull because you are using the lidar to detect the obstacles, you are not doing localization you are not doing lidar sensor model, we are doing obstacle avoidance only
            # you have to transform the lidar data in obstacle positions in the world frame
            # 3. Compute command for the robot with DWA controller
            u = self.compute_cmd(goal_pose, self.robot.pose, self.obstacles) #here we generate the command for the robot

            # 4. Send the command or update the robot pose
            pose = self.robot.update_state(u, self.dt) # in your code you will subscribe to odometry topic and update the robot pose
            
            # 5. Provide intermediate task feedback
            # this is just to check that everything is fine
        ...
        if steps > self.max_num_steps:# questi sono il numero massimo di step che il robot ha a disposizione per raggiungere il goal
            print("Timeout! Goal not reached.") # if I exceed the max number of steps I stop
    ...
    def compute_cmd(self, goal_pose, robot_state, obstacles):
        ...
        return u # you will publish this command to comand_vel topic
        #all this stuff should run at 50 hz in your node
